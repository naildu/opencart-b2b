...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			$date_processing = $this->model_account_order->getOrderStatusAddedTime($order_id, "Processing");
			$date_processing = $date_processing ? $date_processing : date("Y-m-d H:i:s");
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				if($order_status == "Inquiring") {
					...
					$data['text_order_detail'] = $language->get('text_new_inquiry_detail');
				}
				else {
					$subject = sprintf($language->get('text_update_subject'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), $order_id);
					$data['title'] = sprintf($language->get('text_update_subject'), $order_info['store_name'], $order_id);
					$data['text_greeting'] = sprintf($language->get('text_update_greeting'), $order_info['store_name']);
					$data['text_order_detail'] = $language->get('text_order_detail');
				}

				$data['text_appellation'] = $language->get('text_appellation');
				...
				$data['text_inscribe'] = sprintf($language->get('text_inscribe'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
				...
				if ($this->config->get('config_order_mail')) {
					$data['text_appellation'] = '';

					if($order_status == "Inquiring") {
						$data['text_greeting'] = $language->get('text_new_received');
					} else {
						$data['text_greeting'] = "You have updated an order.";
					}
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				$message .= "Best Regards\n" . sprintf($language->get('text_inscribe'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
