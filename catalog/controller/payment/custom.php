...
class ControllerPaymentCustom extends Controller {
	...
	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'custom') {
			$this->load->language('payment/custom');

			$this->load->model('checkout/order');

			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('custom_order_status_id'), '', true);
		}
	}
}