...
class Cart {
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$final_price = $cart['price'] ? $cart['price'] : ($price + $option_price);
				$product_data[] = array(
					...
					'stock'           => $stock,
					'price'           => $final_price,
					'total'           => $final_price * $cart['quantity'],
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0, $price  = null) {
		...
		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', price = " . (is_null($price) ? "NULL" : (float)$price) . ", date_added = NOW()");
		} else {
			$this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + " . (int)$quantity . "), price = " . (is_null($price) ? "NULL" : (float)$price) . " WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
		}
	}
